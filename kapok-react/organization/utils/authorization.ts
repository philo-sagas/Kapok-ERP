...
export function login() {
  new Promise((resolve) => {
    ...
    if (codeChallenge) {
      args.set('code_challenge', String(codeChallenge))
      args.set('code_challenge_method', 'S256')
    }
    window.location.href = authorize_uri + '?' + args
  })
}
...
export function obtainAccessToken(code: string, state: string) {
...
export function obtainIntrospect(tokenType: string = 'access_token') {
  return new Promise<IntrospectType>((resolve, reject) => {
