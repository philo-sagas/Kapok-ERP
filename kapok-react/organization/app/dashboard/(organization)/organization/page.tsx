...
export default function DictionaryPage() {
  ...
  const organizationTypeDict = useDictionary('OrganizationType')
  ...
  function loadData() {
    ...
    setIsLoading(true)
    fetch('/api/organization/v1/organization?' + args, {
      method: 'GET',
      headers: {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      valueGetter: (params: GridValueGetterParams<Number>) => (
        organizationTypeDict.mapping[params.value]
      )
    },
    {
      field: 'enabled',
    ...
    },
    {
      ...
      getActions: (params: GridRowParams) => [
        <GridActionsCellItem icon={<EditIcon fontSize="inherit"/>} onClick={handleEdit} key="Edit" label="Edit"/>,
        <GridActionsCellItem icon={<DeleteIcon fontSize="inherit"/>} onClick={handleDelete} key="Delete"
                             label="Delete"/>
      ]
    }
  ];
    ...
    <>
      ...
      >
        <Breadcrumbs aria-label="breadcrumb" sx={{flexGrow: 1}}>
          ...
          <Typography color="text.primary">组织架构管理</Typography>
        </Breadcrumbs>
        <Box sx={{flexShrink: 1}}>
          <TextField
            label="关键字"
