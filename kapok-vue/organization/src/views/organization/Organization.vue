<template>
  <v-layout full-height>
    <v-app-bar flat border>
      ...
      <v-btn
        :disabled="!hasAuthority('organization:delete')"
    ...
    <v-main>
      ...
      >
        ...
        <template #item.type="{ item }">
          {{ organizationTypeMapping[item.columns.type] }}
        </template>
...
const organizationTypeMapping = dictionaryStore.getMapping('OrganizationType')
...
function loadTree() {
  ...
  axios.get('/api/organization/v1/organization/tree').then(response => {
    const json = response.data
