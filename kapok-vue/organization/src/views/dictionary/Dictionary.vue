...
const state = reactive({
  ...
  headers: [
    {title: '数据字典编码', key: 'code', align: 'start', sortable: true},
    {title: '数据字典名称', key: 'name', align: 'start', sortable: true},
    {title: '数据字典值', key: 'value', align: 'start', sortable: true},
    {title: '数据字典类型', key: 'type', align: 'start', sortable: true},
    {title: '数据字典状态', key: 'status', align: 'start', sortable: false},
...
function handleDelete() {
  ...
  axios.delete(`/api/organization/v1/dictionary/${row.id}`).then(response => {
    const json = response.data
