<template>
  <v-layout full-height>
    <v-app-bar flat border>
      ...
      <v-btn
        :disabled="!hasAuthority('role:delete')"
      ...
      >
        ...
        <template #item.actions="{ item }">
          <v-btn
            ...
            @click="handleGrant(item.raw)">
            <v-icon>mdi-account-key</v-icon>
            <v-tooltip activator="parent" location="top">授权</v-tooltip>
          </v-btn>
          <v-btn
            icon
            size="small"
...
const state = reactive({
  ...
  headers: [
    ...
    {title: '操作', key: 'actions', align: 'center', sortable: false, width: 160}
  ],
  selected: [],
  data: [],
  total: 0
...
function loadData() {
  ...
  axios.get('/api/organization/v1/role', {
    params: {
      ...state.filter,
